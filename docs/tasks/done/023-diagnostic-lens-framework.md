...
- [Milestone 010](./010-visualization-and-ux.md) establishes the current controls, edge visibility, coverage encoding, and graph focus.
...
- [x] Define a typed browser-owned lens identity and deterministic preset model.
- [x] Separate codebase scope, lens preset, advanced overrides, and transient interaction state so they are not one ambiguous state object.
- [x] Add an accessible lens selector in the report shell.
- [x] Implement `Overview`, `Structure`, and derived `Custom` states.
- [x] Teach graph edge rendering to suppress unfocused dependency edges in `Overview` while still drawing the selected or hovered direct neighborhood.
- [x] Render only controls and legends that affect the active lens.
- [x] Keep advanced controls available behind a compact secondary affordance.
- [x] Preserve workspace scope and coherent selection behavior across lens transitions.
- [x] Expose semantic lens state in the DOM for deterministic browser assertions.
- [x] Keep lens derivation in browser presentation and report-view code. Do not add lens fields to `ProjectAnalysis`.
- [x] Keep the browser entrypoint thin by delegating lens transitions to the owning browser components.

## Required tests

- [x] Focused tests cover preset derivation, `Custom` detection, scope preservation, and deterministic restoration.
- [x] Browser coverage proves `Overview` is the initial lens and does not render the complete background dependency graph.
- [x] Browser coverage proves hover and selection still reveal the direct dependency neighborhood in `Overview`.
- [x] Browser coverage proves `Structure` shows containment without dependency focus decoration.
- [x] Browser coverage proves irrelevant controls and legends are absent from each lens.
- [x] Browser coverage proves changing an advanced encoding enters `Custom` and selecting a named lens restores its exact preset.
- [x] Browser coverage proves selection and workspace scope survive compatible lens transitions and clear deliberately when incompatible.
- [x] Existing rendered geometry, collision, coverage-color, and interaction tests remain valid.

## Documentation

- [x] Add `Lens` to the glossary as a browser-owned presentation concept.
- [x] Update the README with implemented lenses, their defaults, and the location of advanced controls.
- [x] Amend ADR 003 only if the implementation changes graph or report-view ownership rather than extending the existing browser presentation boundary. No amendment was needed.

## Verification evidence

- `pnpm checks`: formatting and zero-warning lint passed, type checking passed, all 27 Vitest files and 264 tests passed, both builds passed, all 15 Chromium scenarios passed, the performance sentinel passed, and the built CLI generated the dogfood report.
- Browser inspection of the generated dogfood report confirmed that `Overview` opened with zero background dependency edges, selection revealed only direct relationships, `Structure` removed dependency edges and focus rings while retaining selection, and advanced changes derived `Custom` before named-lens restoration.
