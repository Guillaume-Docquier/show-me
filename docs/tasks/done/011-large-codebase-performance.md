...
- The full benchmark runs as an informational CI step on every push to `main` and writes its measurements and budget comparisons to the GitHub job summary. It remains outside the ordinary local `pnpm checks` gate and cannot block Pages deployment because hosted-runner timing varies. The 300-file sentinel uses broad timing ceilings plus deterministic hashes and collision assertions, making semantic or algorithmic regressions fail without relying on narrow wall-clock variance.
