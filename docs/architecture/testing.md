...
Report-builder tests assert that the safely escaped embedded payload is the complete analysis and contains no presentation-only fields. Browser-presentation tests assert derived renderer-neutral inputs such as representative node sizes and colors, while real-browser tests assert rendered behavior such as hover-driven details, selection restoration, path visibility, and navigation. A visible-node input signature can prove view-state reconstruction, but collision or coordinate behavior requires assertions against rendered geometry. Screenshots are reserved for failures that cannot be identified more precisely.
