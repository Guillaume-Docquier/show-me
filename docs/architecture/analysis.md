...
The analyzed project root may contain one automatically discovered `show-me.config.json` file. This strict JSONC configuration accepts comments and trailing commas; its initial optional `exclude` array represents the complete persisted exclusion-pattern set. Show Me does not search invocation or ancestor directories and does not recognize alternate filenames.
