...
The analyzed project root may contain one automatically discovered `show-me.config.json` file. This strict JSONC configuration accepts comments and trailing commas and can persist output, coverage, and exclusion settings. Show Me does not search invocation or ancestor directories and does not recognize alternate filenames. Output and coverage paths remain CLI concerns; only the optional `exclude` array enters analysis as the complete persisted exclusion-pattern set.

Repeatable CLI `--exclude <pattern>` options represent the same complete set for one invocation. Patterns from either source use gitignore matching against normalized forward-slash project-relative file paths: a pattern without a slash matches basenames at any depth, a slash makes it project-root relative, and a leading slash explicitly anchors it at the project root. For example, an `exclude` value containing only `"*.generated.*"` includes conventional tests and specs while excluding matching generated files. Keeping the defaults and adding that exclusion requires restating all three patterns.

Precedence is applied to the complete value: CLI options replace project configuration, project configuration replaces built-in defaults, and an absent value falls through to the next layer. Defined arrays are never combined, and an explicitly empty array includes conventionally named tests and specs.

Configuration is parsed from JSONC as unknown data, checked by a strict Zod schema, and translated with CLI patterns into the same typed file-selection input before analysis. Empty, multiline, comment, negated, Windows-backslash, absolute-drive, and dot-segment patterns are rejected with source-appropriate errors. Negation is deliberately unsupported because selection can only remove otherwise eligible files. Discovery compiles built-in, configured, and CLI-supplied patterns through the same case-insensitive matcher and tests only supported project files, after permanent directory, `.gitignore`, declaration-file, and language exclusions. Eligible files do not need to be tracked by Git.

Excluded files are filtered before source reading, parsing, line metrics, dependency analysis, coverage matching, and report construction. An included file may resolve an import to an existing excluded file without creating an edge or unresolved-dependency diagnostic.
