...
The workflow uploads only `_site`. The generated report remains `_site/show-me.html`, and an identical `_site/index.html` makes the Pages root useful. A separate deployment job publishes the artifact to the `github-pages` environment.
...
The public report always describes a revision that passed the complete workflow, and the same artifact supports both the explicit report URL and the repository Pages root. A failed quality gate, test, build, analysis, upload, or deployment prevents that revision from replacing the latest successful result.
...
The site shows only the latest successful deployment. Historical reports remain workflow artifacts rather than versioned source files. The two published filenames duplicate the self-contained report in the artifact. Publishing now depends on GitHub Actions and GitHub Pages availability. Action upgrades and the first deployment remain operational checks for maintainers.
